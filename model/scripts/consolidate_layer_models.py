...
def consolidate_layer_models(keyboard_id: int, layer_json_paths: list[Path]) -> dict:
    """Combine a keyboard's rendered <id>_L<n>.json layer files into one object.

    Takes the exact paths Make considers current, rather than globbing a
    directory, so a stale leftover from a shrunk layer count never sneaks in.
    """
