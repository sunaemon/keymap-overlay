...
`LX-08`, `WIN-02`–`WIN-04`, `WIN-07`, and `WIN-08`. KDE window-safety
observations and macOS login probes remain partial evidence requiring the
...
- The Windows session target uses both bundled physical keyboards and the
  installed exact-head Win32 executable. Deterministic firmware commands prove
  nested layer ordering, restoration, ten repeated transitions, and a live
  Vial edit loaded only after process restart. Its native state recorder checks
  the edited `F13` label and final hidden state. A focused Win32 target captures
  standard keyboard reports before, during, and after repeated shows, verifies
  focus retention and topmost/no-activate styles, and receives an injected
  click through the visible overlay. It does not replace visual, display,
...
| Platform `*-04` window safety          | macOS and Windows HIL cover focus, topmost/window styles, and click-through; KDE HIL covers focus assertions                | Pointer pass-through and window order on KDE; complete interactive coverage on GNOME                  | Authorized Wayland pointer injection, active-window assertions, and screen capture                        |
...
| Platform `*-07` typing and identity    | Windows HIL captures standard host key input before/during/after overlay shows and verifies live device IDs                 | Equivalent end-to-end host input on macOS and Linux; physical matrix-switch proof remains `GLOBAL-03` | HIL keyboard plus host input capture on every platform                                                    |
| Platform `*-08` repeated transitions   | `GLOBAL-03` supplies the physical switch proof; macOS, Linux, and Windows HIL cover deterministic KMO sequences             | The release-renderer transition scenario remains on GNOME                                             | Virtual Vial+KMO E2E on GNOME                                                                             |
...
| Platform `*-09` disconnect and arrival | Linux UHID end-to-end test loads a keyboard first connected after daemon startup; reducer and arrival-coalescing unit tests | OS arrival watchers, physical removal, and reconnect on macOS/Linux/Windows                           | Disconnectable virtual HID devices on macOS and Windows exercising the release process                    |
...
5. Add a small HIL rack with both bundled keyboards and controlled USB power.
   ...
   encoder sensor/push coverage, and `GLOBAL-03`.
